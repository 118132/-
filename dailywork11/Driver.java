...
public class Driver {
	...
	private void fillRandom(int[][] A) {//初始化矩阵		
	...
	public static void singleThread(){//串行矩阵乘法计算		
		for(int i=0;i<M;i++){			
			for(int j=0;j<N;j++){				
				for(int k=0;k<K;k++)					
					C[i][j]+=A[i][k]*B[k][j];			
	...
	public static void main(String[] args){		
		...
		long time4=System.currentTimeMillis();//记录结束时间		//打印方法二使用的时间和矩阵C三个随机位置的值		
		...
		for(int i=0;i<M;i++)			
			for(int j=0;j<N;j++)				
				C[i][j]=0;//将C矩阵全置零			
		//建立四个工作线程		
		Thread []poolThreads=new Thread[NUM_THREADS];		for(int i=0;i<NUM_THREADS;i++)			
			poolThreads[i]=new Thread(new WorkThread(i,A,B,C));		//建立线程池		
